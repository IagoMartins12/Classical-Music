# 🚀 GUIA COMPLETO: DEPLOY OPEN-ATLAS EM VPS - ARQUITETURA PROFISSIONAL
...
services:
  mongodb:
    ...
    environment:
      ...
      MONGO_INITDB_DATABASE: open_atlas
    ports:
...
DATABASE_URL=mongodb://admin:SUA_SENHA_SUPER_FORTE_AQUI@localhost:27017/open_atlas?authSource=admin
...
```env
DATABASE_URL=mongodb://app_user:OUTRA_SENHA_FORTE@localhost:27017/open_atlas
```
...
```bash
sudo nano /etc/nginx/sites-available/open-atlas
```
...
server {
    ...
    # Logs
    access_log /var/log/nginx/open-atlas.access.log;
    error_log /var/log/nginx/open-atlas.error.log;
...
server {
    ...
    access_log /var/log/nginx/dev.open-atlas.access.log;
    error_log /var/log/nginx/dev.open-atlas.error.log;
...
# Ativar site
sudo ln -s /etc/nginx/sites-available/open-atlas /etc/nginx/sites-enabled/
...
if [ "$ENVIRONMENT" = "prod" ]; then
    pm2 startOrRestart ecosystem.config.js --only open-atlas-prod
else
    pm2 startOrRestart ecosystem.config.js --only open-atlas-dev
fi
...
# Check PM2 processes
PM2_STATUS=$(pm2 jlist | jq -r '.[] | select(.name=="open-atlas-prod") | .pm2_env.status')
...
# Backup health check daily at 9 AM
0 9 * * * /home/deploy/app/scripts/backup-health.sh
```
...
tail -10 /var/log/nginx/open-atlas.error.log 2>/dev/null || echo "No recent errors"
echo ""
...
# Setup rápido com configurações otimizadas
npm run backup:setup:quick
```

### Passo 2: Configurar Backup Automático

```bash
# Iniciar o scheduler de backup
...
### Passo 3: Configurar Backups Seletivos (Recomendado)

```bash
# Backup completo seletivo (mais eficiente)
npm run backup:quick:all-selective

# Backups específicos
npm run backup:quick:users     # Só usuários
npm run backup:quick:content   # Só conteúdo musical
```

### Passo 4: Automatizar via Cron
...
```
# Backup seletivo diário às 2h
0 2 * * * cd /home/deploy/app/prod && npm run backup:quick:all-selective

# Backup completo semanal aos domingos às 3h
0 3 * * 0 cd /home/deploy/app/prod && npm run backup

# Manutenção semanal às 4h das segundas
0 4 * * 1 cd /home/deploy/app/prod && npm run maintenance:suite

# Monitoramento de backup diário às 9h
0 9 * * * cd /home/deploy/app/prod && npm run backup:monitor
```

### Passo 5: Scripts de Monitoramento e Manutenção

```bash
# Criar script de verificação de backup
nano ~/app/scripts/backup-health.sh
```

```bash
#!/bin/bash

LOG_FILE="/home/deploy/app/logs/backup-health.log"
DATE=$(date '+%Y-%m-%d %H:%M:%S')

echo "[$DATE] Checking backup system health..." >> $LOG_FILE

cd /home/deploy/app/prod

# Verificar status do scheduler
SCHEDULER_STATUS=$(npm run backup:scheduler:status 2>&1)
echo "[$DATE] Scheduler Status: $SCHEDULER_STATUS" >> $LOG_FILE

# Executar diagnóstico de backup
npm run backup:diagnose >> $LOG_FILE 2>&1

# Verificar saúde geral do sistema
npm run maintenance:health >> $LOG_FILE 2>&1

echo "[$DATE] Backup health check completed." >> $LOG_FILE
```

```bash
chmod +x ~/app/scripts/backup-health.sh
```

### Comandos Úteis de Backup

```bash
# Verificar backups disponíveis
npm run backup:list
npm run backup:selective:list

# Backup manual imediato
npm run backup:now

# Restaurar backup específico
npm run backup:restore <caminho_do_backup>
npm run backup:selective:restore <caminho_do_backup>

# Diagnosticar problemas
npm run backup:diagnose
