...
class FamousWorksSubtitleFixer {
  ...
  private famousWorks: FamousWorkPattern[] = [
    ...
    {
      composer: 'Bach',
      patterns: ['Toccata and Fugue in D minor'],
      catalog: 'BWV 565',
      subtitles: [
    ...
    {
      composer: 'Mendelssohn',
      patterns: ['Violin Concerto in E minor'],
      opus: 'Op.64',
      subtitles: [
    ...
    {
      composer: 'Tchaikovsky',
      patterns: ['Piano Concerto No.1 in B-flat minor'],
      opus: 'Op.23',
      subtitles: [
    ...
    {
      composer: 'Grieg',
      patterns: ['Piano Concerto in A minor'],
      opus: 'Op.16',
    ...
    // === SERGEI RACHMANINOFF ===
    {
      ...
      patterns: ['Piano Concerto No.2 in C minor'],
      opus: 'Op.18',
      subtitles: [
    ...
    {
      composer: 'Albéniz',
      patterns: ['Asturias', 'Leyenda', 'Suite Española No.1'],
      subtitles: [
        'Asturias (Leyenda)',
        'Astúrias – Lenda',
        'Suite Española No. 5',
    ...
    {
      composer: 'Mussorgsky',
      patterns: ['Pictures at an Exhibition', 'Quadros de uma Exposição'],
      subtitles: ['Quadros de uma Exposição', 'Pictures at an Exhibition'],
      priority: 85,
    },
    {
      composer: 'Mussorgsky',
      patterns: ['Night on Bald Mountain', 'Noite no Monte Calvo'],
      subtitles: ['Noite no Monte Calvo', 'Night on Bald Mountain'],
      priority: 80,
    },
    ...
    {
      composer: 'Elgar',
      patterns: ['Pomp and Circumstance March No.1', 'Marcha da Coroação'],
      opus: 'Op.39',
      subtitles: [
        'Marcha da Coroação',
        'Marcha Triunfal',
    ...
    {
      composer: 'Dvořák',
      patterns: ['Symphony No.9 in E minor', 'Sinfonia No.9'],
      opus: 'Op.95',
    ...
    {
      composer: 'Dvořák',
      patterns: ['Cello Concerto in B minor'],
      opus: 'Op.104',
      subtitles: [
    ...
    {
      composer: 'Scarlatti',
      patterns: ['Keyboard Sonata in E major, K.380'],
      catalog: 'K.380',
    ...
    // === HEITOR VILLA-LOBOS ===
    {
      ...
      patterns: ['Bachianas Brasileiras No.5', 'Ária'],
  ...
  private matchesFamousWork(
    ...
    try {
      ...
      // ✅ VERIFICAÇÃO RIGOROSA DO COMPOSITOR
      const normalizedPatternComposer = this.normalizeText(pattern.composer);

      // Verificação mais específica - o nome do compositor deve estar presente de forma clara
      const composerMatches = this.isComposerMatch(
        composerName,
        normalizedPatternComposer
      );
      ...
      // ✅ VERIFICAÇÃO DE PADRÕES NO TÍTULO - Mais específica
      for (const titlePattern of pattern.patterns) {
        const normalizedPattern = this.normalizeText(titlePattern);

        // Verificar se o padrão realmente corresponde (não apenas contém)
        if (this.isTitleMatch(workTitle, normalizedPattern)) {
          let matchedBy = `title: "${titlePattern}"`;

          // Verificar opus/catálogo se especificado para maior precisão
      ...
      // ✅ VERIFICAÇÃO POR OPUS/CATÁLOGO - Apenas se for muito específico
      if (pattern.opus && opOrCatalog.includes(pattern.opus)) {
        // Extra verificação: título deve ter alguma similaridade
        const hasRelevantTitle = pattern.patterns.some(
          (p) => workTitle.includes(this.normalizeText(p).split(' ')[0]) // Primeira palavra do padrão
        );
  ...
  // ✅ NOVA FUNÇÃO: Verificação rigorosa do compositor
  private isComposerMatch(
    composerName: string,
    patternComposer: string
  ): boolean {
    if (!composerName || !patternComposer) return false;

    // Lista de apelidos/variações conhecidas para compositores
    const composerVariations: Record<string, string[]> = {
      beethoven: ['beethoven', 'ludwig van beethoven', 'van beethoven'],
      mozart: ['mozart', 'wolfgang amadeus mozart', 'w a mozart'],
      bach: ['bach', 'johann sebastian bach', 'j s bach'],
      chopin: ['chopin', 'frederic chopin', 'frédéric chopin'],
      tchaikovsky: [
        'tchaikovsky',
        'pyotr tchaikovsky',
        'pyotr ilyich tchaikovsky',
      ...
      ravel: ['ravel', 'maurice ravel'],
      liszt: ['liszt', 'franz liszt'],
      schubert: ['schubert', 'franz schubert'],
      brahms: ['brahms', 'johannes brahms'],
      schumann: ['schumann', 'robert schumann'],
      rachmaninoff: [
        'rachmaninoff',
        'sergei rachmaninoff',
        'sergey rachmaninoff',
      ...
      handel: ['handel', 'george frideric handel', 'georg friedrich handel'],
      grieg: ['grieg', 'edvard grieg'],
      mendelssohn: ['mendelssohn', 'felix mendelssohn'],
      dvorak: ['dvorak', 'dvořák', 'antonin dvorak', 'antonín dvořák'],
      sibelius: ['sibelius', 'jean sibelius'],
      satie: ['satie', 'erik satie'],
      ...
      elgar: ['elgar', 'edward elgar'],
      'saint-saens': ['saint-saens', 'saint saens', 'camille saint-saens'],
      gershwin: ['gershwin', 'george gershwin'],
      copland: ['copland', 'aaron copland'],
      barber: ['barber', 'samuel barber'],
      glass: ['glass', 'philip glass'],
      pachelbel: ['pachelbel', 'johann pachelbel'],
      scarlatti: ['scarlatti', 'domenico scarlatti'],
      corelli: ['corelli', 'arcangelo corelli'],
      albéniz: ['albeniz', 'albéniz', 'isaac albeniz', 'isaac albéniz'],
      'villa-lobos': ['villa-lobos', 'villa lobos', 'heitor villa-lobos'],
      nazareth: ['nazareth', 'ernesto nazareth'],
      ...
      stravinsky: ['stravinsky', 'igor stravinsky'],
      bartok: ['bartok', 'bartók', 'bela bartok', 'béla bartók'],
      orff: ['orff', 'carl orff'],
      khachaturian: ['khachaturian', 'aram khachaturian'],
      scriabin: ['scriabin', 'alexander scriabin'],
      shostakovich: ['shostakovich', 'dmitri shostakovich'],
      respighi: ['respighi', 'ottorino respighi'],
      dukas: ['dukas', 'paul dukas'],
      franck: ['franck', 'cesar franck', 'césar franck'],
      gounod: ['gounod', 'charles gounod'],
      massenet: ['massenet', 'jules massenet'],
      falla: ['falla', 'manuel de falla'],
      granados: ['granados', 'enrique granados'],
      purcell: ['purcell', 'henry purcell'],
      salieri: ['salieri', 'antonio salieri'],
    };

    // Verificar se há correspondência exata nas variações
    const patternVariations = composerVariations[patternComposer] || [
      patternComposer,
    ];

    return patternVariations.some((variation) => {
      // Verificação mais rigorosa - deve conter a variação como palavra completa
      const words = composerName.split(/\s+/);
      const variationWords = variation.split(/\s+/);

      // Para compositores com sobrenome único (Bach, Mozart, etc)
      if (variationWords.length === 1) {
        return words.includes(variationWords[0]);
      }

      // Para compositores com nome completo
      return variationWords.every((word) => words.includes(word));
    });
  }

  // ✅ NOVA FUNÇÃO: Verificação rigorosa do título
  private isTitleMatch(workTitle: string, pattern: string): boolean {
    if (!workTitle || !pattern) return false;

    // Padrões que devem ser correspondidos de forma exata ou muito específica
    const exactMatches = [
      'fur elise',
      'para elisa',
      'clair de lune',
    ...
    // Se é um match exato, verificar se o título contém exatamente
    if (exactMatches.includes(pattern)) {
      return workTitle.includes(pattern);
    }

    // Para padrões mais complexos, verificar por palavras-chave específicas
    const patternWords = pattern.split(/\s+/);
    const titleWords = workTitle.split(/\s+/);

    // Padrões numerados (Sonata No.14, Symphony No.5, etc.)
    if (pattern.includes('no.') || pattern.includes('no ')) {
      const numberMatch = pattern.match(/no\.?\s*(\d+)/i);
      if (numberMatch) {
        const number = numberMatch[1];
        const basePattern = pattern.replace(/no\.?\s*\d+/i, '').trim();

        return (
          workTitle.includes(basePattern) &&
          (workTitle.includes(`no.${number}`) ||
            workTitle.includes(`no. ${number}`) ||
            workTitle.includes(`no ${number}`))
        );
      }
    ...
    // Para outros padrões, verificar se contém as palavras principais
    const mainWords = patternWords.filter(
      (word) =>
        word.length > 3 &&
        !['and', 'the', 'in', 'for', 'de', 'la', 'le', 'di'].includes(word)
    );

    if (mainWords.length === 0) return false;

    // Deve conter pelo menos 70% das palavras principais
    const matchingWords = mainWords.filter((word) =>
      titleWords.some(
        (titleWord) => titleWord.includes(word) || word.includes(titleWord)
      )
    );

    return matchingWords.length >= Math.ceil(mainWords.length * 0.7);
  }
