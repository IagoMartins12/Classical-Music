// app/libs/scores-cache-service-incremental.ts - Corrigido para usar WorkScore
...
export class ScoresCacheServiceIncremental {
  private static readonly CACHE_VERSION = '1.3-WORK-SCORE';
  private static readonly DEFAULT_CACHE_TTL = 7 * 24 * 60 * 60 * 1000; // 7 dias

  /**
   * 🚀 Obter partituras incrementais usando WorkScore
   */
  static async getWorkScoresIncremental(
    ...
    try {
      console.log(`💾 [CACHE-WS] Buscando cache para obra ${workId}`);
      console.log(
        `📊 [CACHE-WS] Filtros: types=${specificTypes?.join(',') || 'todos'}`
      );

      // 🆕 Buscar partituras usando WorkScore
      ...
      if (!cachedScores || cachedScores.length === 0) {
        console.log(`❌ [CACHE-WS] Cache miss para obra ${workId}`);
        return {
          scores: null,
          fromCache: false,
          hasEnoughData: false,
      ...
      if (cacheAge > this.DEFAULT_CACHE_TTL) {
        console.log(
          `⏰ [CACHE-WS] Cache expirado para obra ${workId} (${Math.round(
            cacheAge / (1000 * 60 * 60)
          )}h)`
        );
        return {
      ...
      // 🆕 Calcular contadores usando o primeiro score que tem metadados
      ...
      // 🆕 Obter título da obra do primeiro score ou buscar da obra
      ...
      const workScores: IMSLPWorkScoresIncremental = {
        ...
        pagination: {
          ...
          totalPages: Math.ceil(
            Math.max(...Object.values(filteredTotalCounts)) / limit
          ),
          itemsPerPage: limit,
        },
      };
      ...
      console.log(
        `✅ [CACHE-WS] Cache hit: ${totalLoaded}/${totalAvailable} partituras (${totalCached} em cache)`
      );

      return {
        scores: workScores,
        fromCache: true,
        hasEnoughData: totalLoaded >= Math.min(limit, totalAvailable),
      ...
      console.error(`❌ [CACHE-WS] Erro ao buscar cache:`, error);
      return {
        scores: null,
        fromCache: false,
        hasEnoughData: false,
  ...
  static async getTabStatistics(
    ...
    try {
      console.log(
        `📊 [CACHE-WS] Obtendo estatísticas da tab "${tabType}" para obra ${workId}`
      );
      ...
      console.log(
        `📈 [CACHE-WS] Tab "${tabType}": ${loadedForTab}/${totalForTab} (${progress}%)`
      );
      ...
      console.error(`❌ [CACHE-WS] Erro ao obter estatísticas da tab:`, error);
      return {
        loaded: 0,
        total: 0,
        remaining: 0,
  ...
  static async getAllTabsStatistics(workId: string): Promise<TabStatistics[]> {
    try {
      console.log(
        `📊 [CACHE-WS] Obtendo estatísticas de todas as tabs para obra ${workId}`
      );
      ...
      console.log(`📈 [CACHE-WS] Estatísticas de todas as tabs obtidas`);
      return allStats;
    } catch (error) {
      console.error(
        `❌ [CACHE-WS] Erro ao obter estatísticas de todas as tabs:`,
        error
      );
      return [];
    }
  ...
  static async cacheScoresFromIMSLPIncremental(
    ...
    try {
      console.log(
        `💾 [CACHE-WS] Salvando partituras para obra ${workId} (modo: ${
      ...
      const scoresData = [];
      let priority = 1000; // Começar com prioridade alta
        ...
        for (const group of groups) {
          for (const score of group.scores) {
            scoresData.push({
              ...
              imslpTotalCounts: JSON.stringify(scores.totalCounts), // Salvar totais
            });
          }
        }
      }
      ...
      console.log(
        `✅ [CACHE-WS] ${scoresData.length} partituras salvas no cache para obra ${workId}`
      );
    } catch (error) {
      console.error(`❌ [CACHE-WS] Erro ao salvar cache:`, error);
      throw error;
    }
  }
  ...
  static async getCacheProgress(workId: string): Promise<{
    ...
    try {
      ...
      console.error(`❌ [CACHE-WS] Erro ao obter progresso do cache:`, error);
      return {
        workId,
        progress: 0,
        completed: false,
  ...
  static async invalidateWorkCache(workId: string): Promise<void> {
    try {
      console.log(`🗑️ [CACHE-WS] Invalidando cache para obra ${workId}`);
      ...
      console.log(`✅ [CACHE-WS] Cache invalidado para obra ${workId}`);
    } catch (error) {
      console.error(`❌ [CACHE-WS] Erro ao invalidar cache:`, error);
      throw error;
    }
  }
