// components/IMSLPTabsOptimized.tsx
'use client';
...
interface IMSLPTabsOptimizedProps {
  imslpUrl: string;
  workId: string;
  workTitle: string;
  composerName: string;
  onScoreSelect?: (score: IMSLPScore | null) => void;
}
...
interface TabInfo {
  ...
  type: keyof IMSLPWorkScores['totalCounts'];
  gradient: string;
}
...
export default function IMSLPTabsOptimized({
  imslpUrl,
  workId,
  workTitle,
  composerName,
  onScoreSelect,
}: IMSLPTabsOptimizedProps) {
  // Hook otimizado com cache inteligente
  console.log('Work', { workTitle, composerName });
  const {
    scores: imslpData,
    selectedScores,
    loading,
    ...
    refreshScores,
    isScoreSelected,
  } = useIMSLPScoresOptimized(imslpUrl, workId);

  const [activeTab, setActiveTab] = useState<string>(() => {
    // Encontrar a primeira aba com conteúdo
    const firstTabWithContent =
      imslpData && imslpData.totalCounts
        ? TABS.find((tab) => imslpData.totalCounts[tab.type] > 0)
        : undefined;
    return firstTabWithContent?.id || 'scores';
  });

  const [currentSelectedScore, setCurrentSelectedScore] =
    useState<IMSLPScore | null>(null);
  const previewRef = useRef<HTMLDivElement>(null);

  // Gerenciar seleção de partitura
  const handleScoreSelect = async (score: IMSLPScore) => {
    // Se a mesma partitura for clicada, desselecioná-la
    if (currentSelectedScore?.id === score.id) {
      setCurrentSelectedScore(null);
      onScoreSelect?.(null);

      // Remover dos selecionados se estiver salva
      if (isScoreSelected(score.id)) {
        const success = await removeSelectedScore(score.id);
        if (success) {
          toast.success('Partitura removida da seleção');
        }
      }
    } else {
      setCurrentSelectedScore(score);
      onScoreSelect?.(score);

      // Salvar nos selecionados
      const success = await saveSelectedScore(score);
      if (success) {
        toast.success('Partitura selecionada para estudo!');
      } else {
        toast.error('Erro ao selecionar partitura');
      }
    }
  };

  // Quick select de partitura selecionada anteriormente
  const handleQuickSelectSaved = async (savedScore: IMSLPScore) => {
    setCurrentSelectedScore(savedScore);
    onScoreSelect?.(savedScore);
    toast.success('Partitura selecionada!');
  };
    ...
    <div className="classical-card overflow-hidden animate-fade-in-up">
      ...
      <div className="border-b border-theme-secondary bg-gradient-to-r from-theme-primary to-theme-elevated">
        <div className="p-6">
          <div className="flex items-center justify-between">
              ...
              <div>
                ...
                <p className="text-theme-secondary classical-subtitle">
                  {currentSelectedScore
                    ? `Partitura selecionada: ${currentSelectedScore.title}`
            ...
            {currentSelectedScore && (
              <div className="flex items-center space-x-3">
                ...
                <Link
                  href={`/study/${workId}/${currentSelectedScore.id}`}
          ...
          {/* Partituras Selecionadas Anteriormente */}
          {selectedScores.length > 0 && (
            <div className="mt-4 p-4 bg-theme-elevated/30 rounded-xl border border-theme-primary/20">
              <div className="flex items-center space-x-2 mb-3">
                <FiStar className="w-4 h-4 text-accent-yellow" />
                <span className="text-sm font-medium text-theme-primary">
                  Partituras Selecionadas Anteriormente ({selectedScores.length}
                  )
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                {selectedScores.slice(0, 5).map((savedScore) => (
                  <button
                    key={savedScore.id}
                    onClick={() => handleQuickSelectSaved(savedScore)}
                    className={`px-3 py-1 rounded-lg text-xs font-medium transition-all duration-300 ${
                      currentSelectedScore?.id === savedScore.id
                        ? 'bg-brand-primary text-theme-primary'
                        : 'bg-theme-elevated hover:bg-interactive-hover text-theme-secondary hover:text-theme-primary'
                    }`}
                  >
                    {savedScore.title.substring(0, 30)}...
                  </button>
                ))}
                {selectedScores.length > 5 && (
                  <span className="px-3 py-1 text-xs text-theme-tertiary">
                    +{selectedScores.length - 5} mais
                  </span>
                )}
              </div>
            </div>
            ...
            {currentSelectedScore && (
              ...
              >
                <div className="classical-card-2 p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2 text-sm">
                      ...
                      <span className="text-theme-secondary">
                        {isScoreSelected(currentSelectedScore.id)
                          ? 'Salva na biblioteca'
                          : 'Seleção temporária'}
                      </span>
                    </div>
                  ...
                  <ScorePreview score={currentSelectedScore} />
                  ...
                  <div className="mt-4 pt-4 border-t border-theme-secondary">
                    <div className="flex flex-wrap gap-3">
                      <Link
                        href={`/study/${workId}/${currentSelectedScore.id}`}
                        className="btn-classical-primary flex items-center space-x-2 flex-1 min-w-[200px]"
                      >
                        <FiBookOpen className="w-4 h-4" />
                        <span>Abrir Modo Estudo</span>
                      </Link>
                      ...
                      <button
                        onClick={() => handleScoreSelect(currentSelectedScore)}
